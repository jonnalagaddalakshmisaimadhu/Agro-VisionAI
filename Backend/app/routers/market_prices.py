...
async def get_crop_prices(
    ...
    """Get prices for a specific crop, or generate real-time AI estimate if unknown"""
    ...
    if not prices:
        # Fallback to AI Generation for infinite database illusion
        service = MarketPricesService()
        new_price = await service.estimate_price_with_ai(crop_name, db)
        if new_price:
            return [new_price]
            
        raise HTTPException(
            status_code=404,
            detail=f"No prices found for crop: {crop_name} and AI estimation failed."
        )
...
async def get_price_history(
    crops: Optional[List[str]] = Query(["Wheat", "Rice", "Tomato"]),
    db: Session = Depends(get_db)
):
    """Get 6-month price history for charts"""
    service = MarketPricesService()
    history = service.get_monthly_price_history(db, crops)
    return history
